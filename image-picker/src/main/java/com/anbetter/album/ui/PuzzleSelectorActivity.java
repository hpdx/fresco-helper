...
import com.anbetter.album.ui.adapter.PuzzleSelectorAdapter;
import com.anbetter.album.ui.adapter.PuzzleSelectorPreviewAdapter;
...
public class PuzzleSelectorActivity extends AppCompatActivity implements View.OnClickListener, AlbumItemsAdapter.OnClickListener, PuzzleSelectorAdapter.OnClickListener, PuzzleSelectorPreviewAdapter.OnClickListener {
    ...
    private ArrayList<PhotoInfo> selectedPhotoInfos = new ArrayList<>();
...
//        albumModel.query(this, null);
    ...
    private void initPreview() {
        ...
        previewAdapter = new PuzzleSelectorPreviewAdapter(this, selectedPhotoInfos, this);
    ...
    private void initPhotos() {
        ...
        photoInfoList.addAll(albumModel.getCurrAlbumItemPhotos(0));
        photosAdapter = new PuzzleSelectorAdapter(this, photoInfoList, this);
    ...
    public void onClick(View view) {
        ...
        if (R.id.iv_back == id) {
            ...
            PuzzleActivity.startWithPhotos(this, selectedPhotoInfos, Code.REQUEST_PUZZLE, false, Setting.imageEngine);
        }
    }
    ...
    private void updatePhotos(int currAlbumItemIndex) {
        photoInfoList.clear();
        photoInfoList.addAll(albumModel.getCurrAlbumItemPhotos(currAlbumItemIndex));
    ...
    public void onPhotoClick(int position) {
        if (selectedPhotoInfos.size() > 8) {
            Toast.makeText(this, getString(R.string.selector_reach_max_image_hint_easy_photos, 9), Toast.LENGTH_SHORT).show();
            return;
        }

        selectedPhotoInfos.add(photoInfoList.get(position));
        previewAdapter.notifyDataSetChanged();
        rvPreview.smoothScrollToPosition(selectedPhotoInfos.size() - 1);

        tvDone.setText(getString(R.string.selector_action_done_easy_photos, selectedPhotoInfos.size(), 9));
        if (selectedPhotoInfos.size() > 1) {
    ...
    public void onDeleteClick(int position) {
        selectedPhotoInfos.remove(position);
        previewAdapter.notifyDataSetChanged();
        tvDone.setText(getString(R.string.selector_action_done_easy_photos, selectedPhotoInfos.size(), 9));
        if (selectedPhotoInfos.size() < 2) {
