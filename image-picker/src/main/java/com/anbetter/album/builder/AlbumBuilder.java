package com.anbetter.album.builder;
...
import com.anbetter.album.callback.VideoPreviewCallback;
...
public class AlbumBuilder {
     ...
     * 设置默认选择图片集合
     *
     * @param selectedPhotoInfos 默认选择图片集合
     * @return AlbumBuilder
     */
    public AlbumBuilder setSelectedPhotos(ArrayList<PhotoInfo> selectedPhotoInfos) {
        Setting.selectedPhotoInfos.clear();
        if (selectedPhotoInfos.isEmpty()) {
            return AlbumBuilder.this;
        }
        Setting.selectedPhotoInfos.addAll(selectedPhotoInfos);
        Setting.selectedOriginal = selectedPhotoInfos.get(0).selectedOriginal;
        return AlbumBuilder.this;
    }
    ...
    public AlbumBuilder setSelectedPhotoPaths(ArrayList<String> selectedPhotoPaths) {
        ...
        Setting.selectedPhotoInfos.clear();
        ArrayList<PhotoInfo> selectedPhotoInfos = new ArrayList<>();
        for (String path : selectedPhotoPaths) {
            PhotoInfo photoInfo = new PhotoInfo(null, path, 0, 0, 0, 0, 0, null);
            selectedPhotoInfos.add(photoInfo);
        }
        Setting.selectedPhotoInfos.addAll(selectedPhotoInfos);
        return AlbumBuilder.this;
    }
