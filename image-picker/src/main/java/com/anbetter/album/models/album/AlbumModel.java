package com.anbetter.album.models.album;
...
import com.anbetter.album.models.album.entity.AlbumData;
...
//            Log.d(TAG, "call: " + "Empty photos");
            ...
            do {
                ...
                final PhotoInfo imageItem = new PhotoInfo(name, path, dateTime, width, height, size, duration, type);
     ...
     * @return 当前专辑项目的图片集
     */
    public ArrayList<PhotoInfo> getCurrAlbumItemPhotos(int currAlbumItemIndex) {
        return album.getAlbumItem(currAlbumItemIndex).photoInfos;
    }
    ...
    @Deprecated
    public void fillPhoto(Context context, PhotoInfo photoInfo) {
        String filePath = photoInfo.path;
        final File file = new File(photoInfo.path);
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            final int widthCol = cursor.getColumnIndex(MediaStore.MediaColumns.WIDTH);
            final int heightCol = cursor.getColumnIndex(MediaStore.MediaColumns.HEIGHT);
            photoInfo.path = cursor.getString(pathCol);
            photoInfo.type = cursor.getString(mimeTypeCol);
            photoInfo.size = cursor.getLong(sizeCol);
            photoInfo.width = cursor.getInt(widthCol);
            photoInfo.height = cursor.getInt(heightCol);
            photoInfo.name = cursor.getString(nameCol);
            photoInfo.time = cursor.getLong(dateCol);
            photoInfo.duration = cursor.getLong(durationCol);
        }
    }
}
